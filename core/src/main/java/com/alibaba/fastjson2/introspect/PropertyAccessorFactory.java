...
public class PropertyAccessorFactory {
    ...
    public PropertyAccessor create(Method method) {
        return create(method, (p, e) -> (RuntimeException) e);
    }
    ...
    public PropertyAccessor create(Method method, BiFunction<PropertyAccessor, Throwable, RuntimeException> exceptionHandler) {
        ...
        if (parameterCount == 0) {
            return create(BeanUtils.getterName(methodName, null), null, null, method, null, exceptionHandler);
        } else {
            return create(BeanUtils.setterName(methodName, null), null, null, null, method, exceptionHandler);
        }
    }
    ...
    public final PropertyAccessor create(String name, Class<?> propertyClass, Type propertyType, Method getter, Method setter) {
        return create(name, propertyClass, propertyType, getter, setter, (p, e) -> (RuntimeException) e);
    }
    ...
    public <T, V> PropertyAccessor create(
            String name,
            Class<?> propertyClass,
            Type propertyType,
        ...
        return create(name, propertyClass, propertyType, getterFunc, setterFunc, (p, e) -> (RuntimeException) e);
    }
    ...
    public <T, V> PropertyAccessor create(
        ...
        if (propertyClass == BigInteger.class) {
            return new FunctionAccessorBigInteger<T>(name, (Function<T, BigInteger>) getterFunc, (BiConsumer<T, BigInteger>) setterFunc, exceptionHandler);
        }
        if (propertyClass == BigDecimal.class) {
            return new FunctionAccessorBigDecimal<T>(name, (Function<T, BigDecimal>) getterFunc, (BiConsumer<T, BigDecimal>) setterFunc, exceptionHandler);
        }
        if (propertyClass == Boolean.class) {
            return new FunctionAccessorBoolean<T>(name, (Function<T, Boolean>) getterFunc, (BiConsumer<T, Boolean>) setterFunc, exceptionHandler);
        }
        if (propertyClass == Byte.class) {
    ...
    static final class FunctionAccessorBoolean<T> extends FunctionAccessor<T> implements PropertyAccessorBoolean {
        ...
        public FunctionAccessorBoolean(String name, Function<T, Boolean> getterFunc, BiConsumer<T, Boolean> setterFunc, BiFunction<PropertyAccessor, Throwable, RuntimeException> exceptionHandler) {
            super(name, Boolean.class, Boolean.class, getterFunc, setterFunc, exceptionHandler);
            this.getterFunc = getterFunc;
            this.setterFunc = setterFunc;
        }
        @Override
    ...
    static final class FunctionAccessorBigInteger<T> extends FunctionAccessor<T> implements PropertyAccessorBigInteger {
        ...
        public FunctionAccessorBigInteger(
                String name,
                Function<T, BigInteger> getterFunc,
                BiConsumer<T, BigInteger> setterFunc,
                BiFunction<PropertyAccessor, Throwable, RuntimeException> exceptionHandler
        ) {
            super(name, BigInteger.class, BigInteger.class, getterFunc, setterFunc, exceptionHandler);
            this.getterFunc = getterFunc;
            this.setterFunc = setterFunc;
        }
        @Override
    ...
    static final class FunctionAccessorBigDecimal<T> extends FunctionAccessor<T> implements PropertyAccessorBigDecimal {
        ...
        public FunctionAccessorBigDecimal(
                String name,
                Function<T, BigDecimal> getterFunc,
                BiConsumer<T, BigDecimal> setterFunc,
                BiFunction<PropertyAccessor, Throwable, RuntimeException> exceptionHandler
        ) {
            super(name, BigDecimal.class, BigDecimal.class, getterFunc, setterFunc, exceptionHandler);
            this.getterFunc = getterFunc;
            this.setterFunc = setterFunc;
        }
        @Override
