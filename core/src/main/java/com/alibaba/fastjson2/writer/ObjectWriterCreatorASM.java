...
//        }
        ...
        String path = "/Users/wenshao/Downloads/temp/202601/" + className + ".class";
        IOUtils.writeFile(path, code);
    ...
    private void genMethodWrite(
        ...
        hashFilter(mw, fieldWriters);
        mwc.genIsEnabled(JSONWriter.Feature.IgnoreErrorGetter.mask | UnquoteFieldName.mask | BeanToArray.mask);
        mw.ior();
        mw.ifeq(L0);
        ...
        for (FieldWriterGroup group : fieldWriterGroups) {
            if (!group.direct) {
                if (group.start) {
                    ...
                    if (!disableAutoType) {
                        ...
                        String METHOD_DESC_WRITE_TYPE_INFO = "(L" + type.jsonWriterType() + ";Ljava/lang/Object;J)Z";

                        mw.aload(THIS);
                        mw.aload(JSON_WRITER);
                        mw.aload(OBJECT);
                        mw.lload(FEATURES);
                        mw.invokevirtual(TYPE_OBJECT_WRITER_ADAPTER, "writeTypeInfo", METHOD_DESC_WRITE_TYPE_INFO);
                        mw.pop();
                    }
                }
                for (int i = 0; i < group.fieldWriters.size(); i++) {
                    ...
                    gwFieldValue(mwc, fieldWriter, OBJECT, fwr.ordinal, type);
                }
                if (group.end) {
                    mw.aload(1);
            ...
            String METHOD_DESC_WRITER_BUF = "(L" + TYPE_JSON_WRITER + ";)" + type.bufType();

            int slotValueSize = mwc.var("valueSize");
            ...
            for (FieldWriterRecord item : group.fieldWriters) {
                ...
                int nameSize = fieldWriter.nameWithColonUTF8.length;
            ...
            mw.iadd();
            fieldValueCapacity(objectFeatures, group.fieldWriters, mwc, mw, FEATURES, type);
            mw.invokestatic(TYPE_IO, "ensureCapacity", METHOD_DESC_WRITER_BUF_I_BUF);
            mw.astore(BUF);

            ByteArrayEmitter byteArrayEmitter = new ByteArrayEmitter(mw, BUF, OFFSET, mwc.var("quote"));
            ...
            if (group.start) {
                ...
                Label L2 = new Label();
                if ((objectFeatures & WriteClassName.mask) == 0) {
                    mwc.genIsEnabled(WriteClassName.mask, L2);
                }

                int slotTypeInfo = mwc.var("type_info");
                ...
                mw.aload(THIS);
                mw.getfield(TYPE_OBJECT_WRITER_ADAPTER, type.nameWithColon(), type.bufType());
                mw.astore(slotTypeInfo);

                byteArrayEmitter.arrayCopy(slotTypeInfo);
                byteArrayEmitter.incArrayLength(slotTypeInfo);
    ...
    private void gwFieldValueDirect(
        ...
        } else if (Collection.class.isAssignableFrom(Collection.class) && fieldWriter.fieldType instanceof ParameterizedType) {
            Type[] fieldTypeArgs = ((ParameterizedType) fieldWriter.fieldType).getActualTypeArguments();
            if (fieldTypeArgs.length == 1 && fieldTypeArgs[0] == String.class) {
                writeValueMethodName = STRING_VALUE != null ? "writeValueJDK11" : "writeValueJDK8";
            }
        }
    ...
    private static void gwFieldNameDirect(
        ...
        byte[] nameWithColonUTF8 = fieldWriter.nameWithColonUTF8;
        int namelength = nameWithColonUTF8.length; // utf8 & utf16 length is equals

        ByteArrayEmitter emitter = new ByteArrayEmitter(mw, BUF, OFFSET, mwc.var("quote"));
        if (type == JSONWriterType.JSON_UTF8) {
            byte[] fieldNameUTF8 = fieldWriter.fieldName.getBytes(StandardCharsets.UTF_8);
        ...
        mw.lload(FEATURES);
        mw.invokevirtual(TYPE_FIELD_WRITER, type.fieldNameMethodName(), type.fieldNameMethodDesc());
        mw.iconst_0();
        mw.aload(BUF);
        mw.iload(OFFSET);
        mw.iconst_n(namelength);
        mw.invokestatic("java/lang/System", "arraycopy", "(Ljava/lang/Object;ILjava/lang/Object;II)V");
        mw.iinc(OFFSET, namelength);
    }
    ...
    private void genMethodWriteJSONB(
        ...
        for (FieldWriterGroup group : fieldWriterGroups) {
            if (group.direct) {
                ...
                mw.iadd();
                fieldValueCapacity(objectFeatures, group.fieldWriters, mwc, mw, FEATURES, JSONWriterType.JSONB);
    ...
    private void writeFieldValueDirectJSONB(
        ...
        if (fieldClass == boolean.class) {
            ...
            methodName = "writeChar";
            methodDesc = "([BILjava/lang/Character;)I";
        } else if (fieldClass == Byte.class) {
            methodName = "writeInt8";
    ...
    private static void fieldValueCapacity(
        ...
        String TYPE_IO = type.ioType();
        boolean ioInterface = type == JSONWriterType.JSONB;
        /*
        ...
        for (FieldWriterRecord item : fieldWriters) {
            ...
            if (fieldClass == String.class) {
                ...
                mw.aload(FIELD_VALUE);
                mw.invokestatic(TYPE_IO, "stringCapacity", "(Ljava/lang/String;)I", ioInterface);
                mw.iadd();
            } else if (fieldClass == String[].class) {
                int FIELD_VALUE = mwc.var("field_" + item.ordinal);
                mw.aload(FIELD_VALUE);
                ...
                mw.invokestatic(TYPE_IO, "enumCapacity", "(Ljava/lang/Enum;J)I", ioInterface);
                mw.iadd();
                ...
                Class<?> itemClass = fieldWriter.getItemClass();
                if (itemClass == String.class) {
                    mw.invokestatic(TYPE_IO, "stringCapacity", "(Ljava/util/List;)I", ioInterface);
                    mw.iadd();
                } else if (itemClass == Long.class) {
                    mw.invokestatic(TYPE_IO, "int64Capacity", "(Ljava/util/List;)I", ioInterface);
                    mw.iadd();
                } else {
                ...
                Class<?> itemClass = fieldWriter.getItemClass();
                if (itemClass == String.class) {
                    mw.invokestatic(TYPE_IO, "stringCapacity", "(Ljava/util/Collection;)I", ioInterface);
                    mw.iadd();
                } else if (itemClass == Long.class) {
                    mw.invokestatic(TYPE_IO, "int64Capacity", "(Ljava/util/Collection;)I", ioInterface);
                    mw.iadd();
                } else {
                    throw new JSONException("assert error itemClass " + itemClass.getName());
                }
...
//            methodDesc = "(Ljava/lang/String;)V";
    ...
    private static void hashFilter(MethodWriter mw, List<FieldWriter> fieldWriters) {
...
//            mw.ifeq(notNull_);
    ...
    private void gwFieldValueList(
        ...
        if (!disableReferenceDetect) {
            ...
            mw.aload(FIELD_VALUE);
            mw.lload(mwc.var2(CONTEXT_FEATURES));
            mw.invokevirtual(TYPE_FIELD_WRITER, "writeReference", "(L" + TYPE_JSON_WRITER + ";Ljava/lang/Object;Ljava/lang/Object;J)Z");
            mw.ifne(notNull_);
...
//            mw.istore(var(WRITE_NULLS));
    ...
    static boolean supportDirectWrite(long beanFeatures, FieldWriter fieldWriter) {
        ...
        return fieldClass == byte.class || fieldClass == Byte.class
                ...
                || fieldClass == int.class || fieldClass == Integer.class || fieldClass == int[].class
