...
final class ObjectReaderImplHijrahDate
        extends ObjectReaderPrimitive<HijrahDate> {
    static final ObjectReaderImplHijrahDate INSTANCE = new ObjectReaderImplHijrahDate();

    ObjectReaderImplHijrahDate() {
        super(HijrahDate.class);
    }

    @Override
    public HijrahDate readJSONBObject(JSONReader jsonReader, Type fieldType, Object fieldName, long features) {
        String str = jsonReader.readString();
        if (str == null || str.isEmpty()) {
            return null;
        }
        return (HijrahDate) parseChronoLocalDate(str, HijrahChronology.INSTANCE);
    }

    @Override
    public HijrahDate readObject(JSONReader jsonReader, Type fieldType, Object fieldName, long features) {
        String str = jsonReader.readString();
        if (str == null) {
            return null;
        }
