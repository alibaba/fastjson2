...
class JSONReaderUTF8
    ...
    public final Integer readInt32() {
        ...
        if (notMatch) {
            ...
            this.ch = firstChar;
            readNumber0();
            return wasNull ? null : getInt32Value();
        }
...
//                boolean space = false;
        ...
        if (ch == '"' || ch == '\'') {
            ...
            if (valueEscape) {
                ...
                if (strlen == 1) {
                    ...
                    byte[] buf = Arrays.copyOfRange(bytes, start, off);
                    str = STRING_CREATOR_JDK11.apply(buf, LATIN1);
                } else if (ANDROID) {
                    str = getLatin1String(start, off - start);
                } else {
                    str = new String(bytes, start, off - start, ISO_8859_1);
                }
            } else {
                str = new String(bytes, start, off - start, UTF_8);
            }
    ...
    public final boolean nextIfMatchIdent(char c0, char c1, char c2, char c3) {
        ...
        if (offset == this.offset + 3 && ch != '(' && ch != '[' && ch != ']' && ch != ')' && ch != ':' && ch != ',') {
            return false;
        }
    ...
    public final boolean nextIfMatchIdent(char c0, char c1, char c2, char c3, char c4) {
        ...
        if (offset == this.offset + 4 && ch != '(' && ch != '[' && ch != ']' && ch != ')' && ch != ':' && ch != ',') {
            return false;
        }
    ...
    public final boolean nextIfMatchIdent(char c0, char c1, char c2, char c3, char c4, char c5) {
        ...
        if (offset == this.offset + 5 && ch != '(' && ch != '[' && ch != ']' && ch != ')' && ch != ':' && ch != ',') {
            return false;
        }
