...
final class ObjectReaderImplJapaneseDate
        extends ObjectReaderPrimitive<JapaneseDate> {
    static final ObjectReaderImplJapaneseDate INSTANCE = new ObjectReaderImplJapaneseDate();

    ObjectReaderImplJapaneseDate() {
        super(JapaneseDate.class);
    }

    @Override
    public JapaneseDate readJSONBObject(JSONReader jsonReader, Type fieldType, Object fieldName, long features) {
        String str = jsonReader.readString();
        if (str == null || str.isEmpty()) {
            return null;
        }
        return (JapaneseDate) parseChronoLocalDate(str, JapaneseChronology.INSTANCE);
    }

    @Override
    public JapaneseDate readObject(JSONReader jsonReader, Type fieldType, Object fieldName, long features) {
        String str = jsonReader.readString();
        if (str == null) {
            return null;
        }
