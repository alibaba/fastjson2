...
/**
 * Writer for generating CSV (Comma-Separated Values) output from Java objects.
 * Supports writing to various destinations (File, OutputStream, Writer) with
 * automatic type conversion and formatting.
 *
 * <p>The writer supports:
 * <ul>
 *   <li>Automatic quoting of values containing commas, quotes, or newlines</li>
 *   <li>Multiple character encodings (UTF-8, UTF-16, etc.)</li>
 *   <li>Type conversion for primitives, dates, and custom objects</li>
 *   <li>Configurable time zones for date/time values</li>
 *   <li>Streaming output for large datasets</li>
 * </ul>
 *
 * <p><b>Usage Examples:</b></p>
 * <pre>{@code
 * // Writing objects to a file
 * try (CSVWriter writer = CSVWriter.of(new File("output.csv"))) {
 *     writer.writeLine("Name", "Age", "Email");
 *     writer.writeLineObject(new User("John", 30, "john@example.com"));
 *     writer.writeLineObject(new User("Jane", 25, "jane@example.com"));
 * }
 *
...
public abstract class CSVWriter
    ...
    /**
     * Creates a new CSVWriter that writes to an in-memory ByteArrayOutputStream.
     *
    ...
    /**
     * Creates a new CSVWriter that writes to the specified file with the given charset.
     *
     * @param file the file to write to
    ...
    /**
     * Writes an object as a CSV line. The object's fields are extracted using its ObjectWriter
     * and written as comma-separated values.
     *
     * @param object the object to write, or null to write an empty line
     */
    ...
    /**
     * Writes a Date value in ISO format.
     *
     * @param date the date to write, or null to write nothing
     */
    ...
    /**
     * Writes an Instant value in ISO format.
     *
     * @param instant the instant to write, or null to write nothing
     */
    ...
    /**
     * Writes a LocalDate value in ISO format (yyyy-MM-dd).
     *
     * @param date the date to write, or null to write nothing
     */
    ...
    /**
     * Writes a LocalDateTime value in ISO format.
     *
     * @param instant the date-time to write, or null to write nothing
     */
...
//    protected abstract void writeDirect(byte[] bytes, int off, int len);
    ...
    /**
     * Writes a line with values generated by a function for each column index.
     *
     * @param columnCount the number of columns to write
     * @param function the function that provides values for each column index
     */
    ...
    /**
     * Writes a line with values from a List.
     *
     * @param values the list of values to write as a CSV line
     */
    ...
    /**
     * Writes a line with values from a varargs array.
     *
     * @param values the values to write as a CSV line
     */
